...
/**
 * AnimatedThemeToggle — sun/moon animated toggle.
 *
 * When the site is in DARK mode → shows the SUN (indicating "click to go light").
 * When the site is in LIGHT mode → shows the MOON (indicating "click to go dark").
 *
 * Integrates with ChenEYE's `data-theme` attribute on <html>.
 */
export const AnimatedThemeToggle = ({
  ...
  return (
    <ShadcnButton
      onClick={toggleTheme}
      className={cn(
        "px-2.5 rounded-xl border-0 bg-transparent shadow-none cursor-pointer transition-all duration-200",
        scrolled
